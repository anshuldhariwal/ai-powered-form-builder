...
import { MandatoryBuilder, MandatoryPublicForm } from './mandatory.jsx';
...
function AuthForm({ mode }) {
    ...
    async function submit(event) {
        ...
        try { await api(isRegistering ? '/register' : '/login', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(event.currentTarget))) }); const next = new URLSearchParams(window.location.search).get('next'); window.location.assign(next?.startsWith('/') ? next : '/'); }
...
function Dashboard() {
    const [data, setData] = useState(null); const [error, setError] = useState(''); const [membersOpen, setMembersOpen] = useState(false);
    function reload() { api('/api/forms').then(setData).catch((e) => setError(e.message)); }
    useEffect(reload, []);
    ...
    async function importForm(file) { if (!file) return; setError(''); try { const body = new FormData(); body.append('file', file); const upload = await fetch('/api/imports', { method: 'POST', credentials: 'same-origin', headers: { Accept: 'application/json', 'X-CSRF-TOKEN': csrfToken }, body }); let request = await upload.json(); if (!upload.ok) throw new Error(Object.values(request.errors ?? {}).flat().join(' ') || request.message); for (let attempt = 0; attempt < 60 && !['ready', 'failed'].includes(request.status); attempt += 1) { await new Promise((resolve) => setTimeout(resolve, 1000)); request = await api(`/api/imports/${request.public_id}`); } if (request.status !== 'ready') throw new Error(request.error_message || 'Import failed.'); const edited = window.prompt(`Review warnings: ${(request.warnings ?? []).join(' ')}\nEdit the candidate JSON if needed, then press OK to commit.`, JSON.stringify(request.candidate_schema, null, 2)); if (!edited) return; const form = await api(`/api/imports/${request.public_id}/commit`, { method: 'POST', body: JSON.stringify({ schema: JSON.parse(edited) }) }); window.location.assign(`/forms/${form.public_id}`); } catch (e) { setError(e.message); } }
    async function switchTenant(slug) { setError(''); try { await api(`/api/tenants/${encodeURIComponent(slug)}/switch`, { method: 'POST' }); reload(); } catch (e) { setError(e.message); } }
    async function changeStatus(form, action) { setError(''); if (action === 'archive' && !window.confirm(`Archive “${form.title}”? Its public link will stop working.`)) return; try { await api(`/api/forms/${form.public_id}/${action}`, { method: 'POST' }); reload(); } catch (e) { setError(e.message); } }
    async function logout() { await api('/logout', { method: 'POST' }); window.location.assign('/login'); }
    const canManage = ['owner', 'editor'].includes(data?.current_role);
    return <main className="min-h-screen bg-slate-950 p-6 text-slate-100"><div className="mx-auto max-w-6xl"><header className="flex flex-wrap items-center justify-between gap-4 py-6"><div><Brand/><div className="mt-2 flex items-center gap-3"><h1 className="text-3xl font-bold">{data?.tenant?.name ?? 'Your forms'}</h1>{data?.tenants?.length > 1 && <select aria-label="Active workspace" value={data.tenant.slug} onChange={(event) => switchTenant(event.target.value)} className="rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm">{data.tenants.map((tenant) => <option key={tenant.slug} value={tenant.slug}>{tenant.name}</option>)}</select>}<span className="rounded-full bg-slate-800 px-2 py-1 text-xs uppercase">{data?.current_role}</span></div></div><div className="flex flex-wrap gap-3">{data?.current_role === 'owner' && <button onClick={() => setMembersOpen(true)} className="rounded-xl border border-slate-700 px-4 py-3 font-bold">Members</button>}{canManage && <><label className="cursor-pointer rounded-xl border border-slate-700 px-5 py-3 font-bold">Import DOCX/XLSX<input type="file" accept=".docx,.xlsx" className="hidden" onChange={(event) => importForm(event.target.files[0])} /></label><button onClick={generateForm} className="rounded-xl border border-cyan-400 px-5 py-3 font-bold text-cyan-300">Generate with AI</button><button onClick={createForm} className="rounded-xl bg-cyan-400 px-5 py-3 font-bold text-slate-950">New form</button></>}<button onClick={logout} className="rounded-xl border border-slate-700 px-4">Log out</button></div></header>{error && <p className="mb-4 text-rose-400">{error}</p>}{membersOpen && <MembersPanel close={() => setMembersOpen(false)} />}{!canManage && <p className="mb-4 rounded-xl border border-slate-800 bg-slate-900 p-3 text-sm text-slate-400">You have read-only access to this workspace.</p>}<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{data?.forms?.map((form) => <article key={form.public_id} className={`rounded-2xl border bg-slate-900 p-5 ${form.status === 'archived' ? 'border-slate-800 opacity-70' : 'border-slate-800 hover:border-cyan-500'}`}><div className="flex justify-between gap-3"><a href={`/forms/${form.public_id}`} className="font-bold hover:text-cyan-300">{form.title}</a><span className="rounded-full bg-slate-800 px-2 py-1 text-xs uppercase">{form.status}</span></div><p className="mt-6 text-sm text-slate-400">Version {form.current_version?.version_number ?? 1}</p>{canManage && <div className="mt-4 flex gap-3 text-sm">{form.status === 'archived' ? <button onClick={() => changeStatus(form, 'restore')} className="text-cyan-300">Restore</button> : <><button onClick={() => changeStatus(form, 'archive')} className="text-rose-300">Archive</button>{form.status === 'published' && <button onClick={() => changeStatus(form, 'unpublish')} className="text-amber-300">Unpublish</button>}</>}</div>}</article>)}</div>{data?.forms?.length === 0 && <p className="mt-16 text-center text-slate-400">{canManage ? 'Create your first form to begin.' : 'This workspace has no forms.'}</p>}</div></main>;
}

function MembersPanel({ close }) {
    const [data, setData] = useState(null); const [error, setError] = useState(''); const [inviteUrl, setInviteUrl] = useState('');
    function reload() { api('/api/tenant/members').then(setData).catch((issue) => setError(issue.message)); }
    useEffect(reload, []);
    async function invite(event) { event.preventDefault(); setError(''); setInviteUrl(''); try { const result = await api('/api/tenant/invitations', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(event.currentTarget))) }); setInviteUrl(result.accept_url); event.currentTarget.reset(); reload(); } catch (issue) { setError(issue.message); } }
    async function updateMember(id, role) { try { await api(`/api/tenant/members/${id}`, { method: 'PATCH', body: JSON.stringify({ role }) }); reload(); } catch (issue) { setError(issue.message); } }
    async function removeMember(member) { if (!window.confirm(`Remove ${member.name} from this workspace?`)) return; try { await api(`/api/tenant/members/${member.id}`, { method: 'DELETE' }); reload(); } catch (issue) { setError(issue.message); } }
    return <section className="mb-6 rounded-2xl border border-cyan-900 bg-slate-900 p-5"><div className="flex justify-between"><h2 className="text-xl font-bold">Workspace members</h2><button onClick={close} className="text-cyan-300">Close</button></div>{error && <p className="mt-3 text-rose-400">{error}</p>}<form onSubmit={invite} className="mt-4 grid gap-2 md:grid-cols-[1fr_140px_auto]"><input required name="email" type="email" placeholder="teammate@example.com" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"/><select name="role" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2"><option value="editor">Editor</option><option value="viewer">Viewer</option></select><button className="rounded-lg bg-cyan-400 px-4 py-2 font-bold text-slate-950">Create invite</button></form>{inviteUrl && <div className="mt-3 rounded-lg bg-slate-950 p-3 text-sm"><p className="text-emerald-300">Invite created. Copy this single-use link:</p><div className="mt-2 flex gap-2"><input readOnly value={inviteUrl} className="min-w-0 flex-1 bg-transparent text-slate-300"/><button onClick={() => navigator.clipboard.writeText(inviteUrl)} className="text-cyan-300">Copy</button></div></div>}<div className="mt-5 space-y-2">{data?.members?.map((member) => <div key={member.id} className="flex flex-wrap items-center gap-3 rounded-lg bg-slate-800 p-3"><div className="min-w-52 flex-1"><p className="font-bold">{member.name}</p><p className="text-sm text-slate-400">{member.email}</p></div><select value={member.pivot.role} onChange={(event) => updateMember(member.id, event.target.value)} className="rounded bg-slate-950 p-2"><option value="owner">Owner</option><option value="editor">Editor</option><option value="viewer">Viewer</option></select><button onClick={() => removeMember(member)} className="text-rose-300">Remove</button></div>)}</div>{data?.invitations?.length > 0 && <div className="mt-5"><h3 className="font-bold">Pending invitations</h3>{data.invitations.map((invitation) => <p key={invitation.public_id} className="mt-2 text-sm text-slate-400">{invitation.email} · {invitation.role} · expires {new Date(invitation.expires_at).toLocaleDateString()}</p>)}</div>}</section>;
}

function InvitationPage({ publicId }) {
    const [message, setMessage] = useState(''); const [error, setError] = useState(''); const tokenValue = new URLSearchParams(window.location.search).get('token');
    async function accept() { setError(''); try { const result = await api(`/api/invitations/${publicId}/accept`, { method: 'POST', body: JSON.stringify({ token: tokenValue }) }); setMessage(`You joined ${result.tenant.name}.`); setTimeout(() => window.location.assign('/'), 800); } catch (issue) { setError(issue.message); } }
    return <main className="grid min-h-screen place-items-center bg-slate-950 p-6 text-white"><div className="w-full max-w-lg rounded-3xl border border-slate-800 bg-slate-900 p-8 text-center"><Brand/><h1 className="mt-4 text-3xl font-bold">Workspace invitation</h1><p className="mt-3 text-slate-400">Sign in with the invited email address, then accept this invitation.</p>{message && <p className="mt-4 text-emerald-300">{message}</p>}{error && <p className="mt-4 text-rose-300">{error}</p>}<div className="mt-6 flex justify-center gap-3"><a href={`/login?next=${encodeURIComponent(window.location.pathname + window.location.search)}`} className="rounded-xl border border-slate-700 px-5 py-3">Sign in</a><button disabled={!tokenValue} onClick={accept} className="rounded-xl bg-cyan-400 px-5 py-3 font-bold text-slate-950 disabled:opacity-50">Accept invitation</button></div></div></main>;
}
...
export function PublicForm({ tenantSlug, formSlug }) {
...
export default function App() {
    ...
    if (path.startsWith('/invitations/')) return <InvitationPage publicId={path.split('/')[2]}/>;
    if (path.startsWith('/forms/')) return <MandatoryBuilder publicId={path.split('/')[2]}/>;
    if (path.startsWith('/f/')) { const [, , tenantSlug, formSlug] = path.split('/'); return <MandatoryPublicForm tenantSlug={tenantSlug} formSlug={formSlug}/>; }
    return <Dashboard/>;
}
