...
export function MandatoryBuilder({ publicId }) {
    ...
    async function showVersions() {
        try { const versions = await api(`/api/forms/${publicId}/versions`); const selected = window.prompt(`Version history: ${versions.map((item) => item.version_number).join(', ')}\nEnter a version number to restore as a new version.`); if (!selected) return; const result = await api(`/api/forms/${publicId}/versions/${selected}/rollback`, { method: 'POST' }); const refreshed = await api(`/api/forms/${publicId}`); commit(refreshed.current_version.schema_json); setNotice(`Rolled version ${selected} forward as version ${result.version.version_number}`); } catch (issue) { setError(issue.message); }
    }
    function addCondition() {
        const allFields = schema.steps.flatMap((item) => item.sections.flatMap((part) => part.fields)).filter((field) => field.type !== 'heading');
        if (allFields.length < 2) { setError('Add at least two input fields before creating a condition.'); return; }
        const source = window.prompt(`Source field key (${allFields.map((field) => field.key).join(', ')})`, allFields[0].key); if (!source) return;
        const target = window.prompt('Target field key', allFields[1].key); if (!target) return;
        const value = window.prompt('Show the target when the source equals:', 'yes'); if (value === null) return;
        const targetField = allFields.find((field) => field.key === target); if (!targetField) { setError('Unknown target field.'); return; }
        commit({ ...schema, conditions: [...schema.conditions, { id: token('condition'), source_field_key: source, operator: 'equals', value, target_type: 'field', target_id: targetField.id, action: 'show' }] });
    }
    ...
    return <main className="min-h-screen bg-slate-950 text-slate-100">
        ...
        <div className="mx-auto grid max-w-[1500px] gap-5 p-5 lg:grid-cols-[250px_1fr]">
            <aside className="space-y-5 rounded-2xl border border-slate-800 bg-slate-900 p-4">
                ...
                <div><h2 className="mb-2 font-bold">Conditions ({schema.conditions.length})</h2><button onClick={addCondition} className="text-sm text-cyan-300">+ Add show rule</button>{schema.conditions.map((condition, index) => <div key={condition.id} className="mt-2 rounded bg-slate-800 p-2 text-xs"><p>{condition.source_field_key} {condition.operator} {String(condition.value)}</p><button onClick={() => commit({ ...schema, conditions: schema.conditions.filter((_, position) => position !== index) })} className="text-rose-400">Remove</button></div>)}</div>
...
export function MandatoryPublicForm({ tenantSlug, formSlug }) {
    ...
    function conditionMatches(condition) { const value = answers[condition.source_field_key]; if (condition.operator === 'equals') return String(value ?? '') === String(condition.value ?? ''); if (condition.operator === 'not_equals') return String(value ?? '') !== String(condition.value ?? ''); if (condition.operator === 'is_empty') return value === undefined || value === ''; if (condition.operator === 'is_not_empty') return value !== undefined && value !== ''; if (condition.operator === 'contains') return String(value ?? '').includes(String(condition.value ?? '')); if (condition.operator === 'greater_than') return Number(value) > Number(condition.value); if (condition.operator === 'less_than') return Number(value) < Number(condition.value); return false; }
    function fieldState(field) { let visible = !schema.conditions.some((condition) => condition.target_id === field.id && condition.action === 'show'); let required = field.required; schema.conditions.filter((condition) => condition.target_id === field.id && conditionMatches(condition)).forEach((condition) => { if (condition.action === 'show') visible = true; if (condition.action === 'hide') visible = false; if (condition.action === 'require') required = true; if (condition.action === 'optional') required = false; }); return { visible, required }; }
    return <main className="min-h-screen bg-slate-950 p-6 text-slate-100"><form onSubmit={(event) => { event.preventDefault(); if (step < total - 1) setStep(step + 1); else submit(); }} className="mx-auto max-w-2xl space-y-5 rounded-3xl border border-slate-800 bg-slate-900 p-8"><p className="text-sm font-black uppercase tracking-[.22em] text-cyan-400">FormForge AI</p><h1 className="text-4xl font-bold">{schema.form.title}</h1>{schema.settings.show_progress && <div className="h-2 rounded bg-slate-800"><div className="h-2 rounded bg-cyan-400" style={{ width: `${((step + 1) / total) * 100}%` }} /></div>}<h2 className="text-2xl font-bold">{current.title}</h2>{current.sections.map((section) => <section key={section.id} className="space-y-4"><h3 className="text-lg font-semibold text-cyan-200">{section.title}</h3>{section.fields.map((field) => { const state = fieldState(field); return state.visible ? <LiveField key={field.id} field={{ ...field, required: state.required }} value={answers[field.key]} setValue={(value) => setAnswers({ ...answers, [field.key]: value })} /> : null; })}</section>)}{error && <p className="text-rose-400">{error}</p>}<div className="flex justify-between">{step > 0 ? <button type="button" onClick={() => setStep(step - 1)} className="rounded-xl border border-slate-700 px-4 py-3">Back</button> : <span />}<button className="rounded-xl bg-cyan-400 px-6 py-3 font-bold text-slate-950">{step < total - 1 ? 'Next' : schema.form.submit_label}</button></div></form></main>;
}
